...
void print_object_state( pjson_data& d, const char* label )
{
    ...
    {
        pjson_kv_pair& pair = AddressManager<pjson_kv_pair>::GetArrayElement( pairs_slot, i );

        unsigned    chars_slot = pair.key.chars.addr();
        const char* keystr = ( chars_slot != 0 ) ? &AddressManager<char>::GetArrayElement( chars_slot, 0 ) : "(null)";
...
void trace_lower_bound( pjson_data& d, const char* search_key )
{
    ...
    while ( lo < hi )
    {
        ...
        pjson_kv_pair& pair       = AddressManager<pjson_kv_pair>::GetArrayElement( pairs_slot, mid );
        unsigned       chars_slot = pair.key.chars.addr();
        const char*    k          = ( chars_slot != 0 ) ? &AddressManager<char>::GetArrayElement( chars_slot, 0 ) : "";
    ...
    if ( lo < sz )
    {
        pjson_kv_pair& pair       = AddressManager<pjson_kv_pair>::GetArrayElement( pairs_slot, lo );
        unsigned       chars_slot = pair.key.chars.addr();
        const char*    k = ( chars_slot != 0 ) ? &AddressManager<char>::GetArrayElement( chars_slot, 0 ) : "(null)";
...
int main()
{
    ...
    {
        unsigned       pairs_slot = d.payload.object_val.pairs_slot;
        pjson_kv_pair& pair_c     = AddressManager<pjson_kv_pair>::GetArrayElement( pairs_slot, 1 );
    ...
    for ( const char* key : { "a", "b", "c" } )
    {
        ...
        if ( found )
        {
            printf( "  obj_find(\"%s\") -> FOUND  value=%lld\n", key,
                    static_cast<long long>( pjson( *found ).get_int() ) );
        }
        else
        {
