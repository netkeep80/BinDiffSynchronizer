...
| **Метапрограммирование** | Устранение внутреннего дублирования кода через шаблонные helpers (Фаза 16); `node_resolve_and_set_tag()` — единый boilerplate для `node_set_*` (Задача 16.1); `node_set_container_empty()` — единая инициализация контейнерных узлов (Задача 16.1) |
