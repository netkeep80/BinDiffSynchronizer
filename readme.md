...
Конечная цель — реализация `pjson`, персистного JSON-хранилища без внешних зависимостей.
...
| `pjson.h` | Персистный JSON `pjson`: хранение и обход JSON-значений в ПАП |
| `pjson_serializer.h` | Прямая сериализация/десериализация pjson (без внешних зависимостей) |
| `pjson_interning.h` | Интернирование строк для pjson (дедупликация, F3) |
| `pjson_node_pool.h` | Пул узлов pjson для быстрой аллокации (F2) |
