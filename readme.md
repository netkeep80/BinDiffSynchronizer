...
Задача 4.1 (регрессионное тестирование) выполнена: 606 тестов, 354 101 assertion.
Оставшиеся задачи: 4.2 (совместимость save/load), 4.3 (бенчмарки).

### Устранение дублирования кода (Этап 5, Issue #128)

Проведён code review, выявлено 10 паттернов дублирования (~256 строк, 38 повторений).
Составлен план поэтапного рефакторинга с использованием метапрограммирования (C++20 шаблоны,
CRTP, visitor-паттерн). Ключевые направления:

- Единый `_lower_bound()` для бинарного поиска в `pmap_pmm` (4 → 1 реализация)
- Шаблонный итератор `iterator_base<IsConst>` (2 → 1 класс)
- `_put_impl()` шаблонный helper для put-методов (7 → 1 реализация)
- Visitor-обход `traverse_subtree()` для рекурсивных операций (4 → 1 реализация)
