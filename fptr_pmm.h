...
/**
 * @file fptr_pmm.h
 * @brief Персистный указатель на базе PMM (Задача 14.7).
 *
 * Этот файл реализует fptr_pmm<T> — персистный указатель, совместимый по API
 * с оригинальным fptr<T> из persist.h, но использующий PMM в качестве бэкенда.
 *
 * Ключевые отличия от оригинального fptr<T>:
 ...
 *   - Сохраняет размер sizeof(fptr_pmm<T>) == sizeof(void*) (Тр.5)
 *
 ...
 *
 * Реализует Тр.5, Тр.9, Тр.12, Тр.13, Тр.15 из persist.h, но использует
 * PMM (pam_pmm.h) в качестве бэкенда вместо PersistentAddressSpace.
 *
...
/**
 * @brief Алиас fptr<T> = fptr_pmm<T> для упрощения миграции.
 *
 * Используйте pjson::fptr<T> в новом коде для работы с PMM-бэкендом.
 * Оригинальный ::fptr<T> из persist.h использует PersistentAddressSpace.
 */
