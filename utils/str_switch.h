...
{
    return str.size() ? ( static_cast<signed char>( str[0] ) > 0 ) && str_is_correct( str.substr( 1, str.size() - 1 ) )
                      : true;
}
...
{
    return *str
               ? ( raise_128_to( current_len - 1 ) * static_cast<uchar>( *str ) + str_hash( str + 1, current_len - 1 ) )
               : 0;
}

constexpr ullong str_hash( const std::string_view& str )
{
    return str.size() ? ( raise_128_to( str.size() - 1 ) * static_cast<uchar>( str[0] ) +
                          str_hash( str.substr( 1, str.size() - 1 ) ) )
                      : 0;
}
...
{
    return ( str_is_correct( str.c_str() ) && ( str.length() <= MAX_LEN ) ) ? str_hash( str.c_str(), str.length() )
                                                                            : N_HASH;
}
