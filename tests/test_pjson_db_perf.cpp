...
//
// Проверяют, что высокоуровневый API pjson_db_pmm справляется с 10k узлами
// за приемлемое время.
//
// Покрытие:
//   - put() 1k целочисленных значений по уникальным путям
//   - put() 1k строковых значений
//   - get() 10k запросов (чтение по 10k узлам, каждый LARGE_N/PERF_N раз)
...
// плоской структуры используется PERF_N = 1k (соответствует требованиям задачи 9.2).
