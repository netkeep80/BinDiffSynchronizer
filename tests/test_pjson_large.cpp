...
// ---------------------------------------------------------------------------
static uintptr_t nlohmann_to_pjson_limited( const json& src, uintptr_t dst_offset, int max_depth, uintptr_t& node_count,
                                            uintptr_t max_nodes )
{
    auto&  pam = PersistentAddressSpace::Get();
    pjson* dst = pam.Resolve<pjson>( dst_offset );
...
// ---------------------------------------------------------------------------
TEST_CASE( "pjson large: test.json loads correctly via nlohmann::json", "[pjson][large][json]" )
{
...
TEST_CASE( "pjson large: first keys from test.json are stored in pjson", "[pjson][large][json]" )
{
    ...
    for ( const auto& [key, val] : nlohmann_root.items() )
    {
        ...
        if ( found == nullptr )
            break; // дошли до конца конвертированной части
        verified++;
        if ( verified >= keys_processed )
            break;
    }
    REQUIRE( verified > 0 );
