...
//
// Проверяет:
//   — единое ПАП для объектов разных типов
//   — при загрузке образа конструкторы не вызываются
//   — ПАМ хранит имена объектов
//   — поиск по имени
//   — ПАМ хранит карту объектов
// =============================================================================
...
// ---------------------------------------------------------------------------
// Единое ПАП для разных типов
// ---------------------------------------------------------------------------
TEST_CASE( "pam_pmm: int and double objects in unified PAP", "[pam][unified_space]" )
{
    pam_pmm_init( nullptr );
...
// ---------------------------------------------------------------------------
// Save и повторная загрузка через Init (без конструкторов)
// ---------------------------------------------------------------------------
TEST_CASE( "pam_pmm: Save and Init -- objects are restored", "[pam][save_reload]" )
{
    ...
    // Перезагружаем ПАП из файла. Конструкторы НЕ вызываются.
    pam_pmm_init( fname );
    {
