...
1. **Реализовать систему коммитов и веток** jgit
2. **Интегрировать JSON Patch** (RFC 6902) как формат дельт на базе BinDiffSynchronizer
3. **Поддержка `$ref`-ссылок** через `fptr<T>`
4. **Создать CLI jgit** с командами init, commit, log, diff, checkout
5. **Опубликовать статью** или документацию о концепции jgit
