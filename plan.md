...
### Задача 16.2. Извлечение бинарного поиска по объектным ключам

**Цель:** Устранить дублирование бинарного поиска в `node_view::at(const char*)`,
`node_object_insert()` и `pjson_db_pmm::_object_erase()`.

- [ ] Добавить helper-функцию `node_object_find_key()` — бинарный поиск по ключу в массиве `object_entry`.
- [ ] Делегировать все три реализации через этот helper.
- [ ] Прогнать все тесты.

---

### Задача 16.3. Шаблонный helper для безопасного разрешения узла с проверкой тега
...
- [ ] Добавить `node_resolve_checked()` — helper для разрешения с проверкой тега.
- [ ] Делегировать повторяющиеся паттерны через helper.
- [ ] Прогнать все тесты.
...
- [ ] Создать массив-реестр метрик: пара (имя, lambda/функция для чтения значения).
- [ ] Заменить цепочку strcmp на цикл по реестру.
- [ ] Прогнать все тесты.

---

### Задача 16.5. Обновить тесты, CI и документацию

- [ ] Убедиться, что все тесты проходят.
- [ ] Обновить `readme.md` (добавить Фазу 16 в таблицу характеристик).
- [ ] Обновить `plan.md` — критерии приёмки и статус задач.

---

### Критерии приёмки Фазы 16

- [ ] Все шаблонные helpers — `inline` в header-файлах (header-only).
- [ ] Публичный API не изменён — все существующие функции сохранены.
- [ ] Нет дублирования boilerplate-кода в `node_set_*` функциях.
- [ ] Бинарный поиск по объектным ключам — единственная реализация.
- [ ] Все 711+ тестов проходят.
- [ ] CI проходит.
- [ ] `plan.md` обновлён.
