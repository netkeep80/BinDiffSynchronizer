...
|---------|-----|----------|-------------------|
| Файлы в уровне A | 5 | 1–2 | 4 (pam_adapter.h ожидает pmm 4.4) |
| Файлы в уровне B | 5 | 1 (pstringview_pmm wrapper) | 2 (pstringview_pmm + pmap_pmm — тонкие обёртки) |
| Строки кода (уровни A+B) | ~2500 | ~300 | ~832 (обёртки, без pam_pmm.h фасада) |
| Собственные персистентные типы | 7 | 1–2 (тонкие обёртки) | 2 (pmap_pmm sorted array, pstringview_pmm алиасы) |

**Удалённые файлы:** `pallocator_pmm.h`, `pstring_pmm.h`, `pmem_array_pmm.h`, `pvector_pmm.h` — 4 файла, ~1700 строк кода.
...
- ⏳ **Полное удаление адаптера** — `pam_adapter.h` будет удалён после pmm 4.4
...
pmm Фаза 3 (параллельно с планированием миграции)
    ...
    │
    └── pmm 4.4 byte offset ─────── Этап 1.1 (удаление pam_adapter)      ⏳ ожидает pmm 4.4
                                         │
...
Этапы 1 (кроме 1.1) и 2 выполнены. Обновление типов в этапе 3 было выполнено
параллельно с этапом 2, т.к. замена типов в node/codec/db была частью каждой задачи этапа 2.

**Оставшаяся задача:** Этап 1.1 — удаление `pam_adapter.h` — заблокирован: требуется
pmm 4.4 (`pptr::byte_offset()` и `PersistMemoryManager::pptr_from_byte_offset<T>()`).
