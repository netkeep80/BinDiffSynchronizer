...
## Фаза 17: Пакетные операции (batch API)

### Задача 17.1: batch_begin / batch_end и RAII-обёртка batch_guard ✅

Добавлен API для пакетных операций, откладывающий пересчёт метрик до завершения пакета:

- **`batch_begin()` / `batch_end()`** — ручное управление пакетной областью с поддержкой вложенности.
- **`in_batch()`** — проверка, активна ли пакетная операция.
- **`batch_guard`** — RAII-класс, автоматически вызывающий `batch_begin()` в конструкторе и `batch_end()` в деструкторе.
- **`batch()`** — фабричный метод для создания `batch_guard`.

Пока активна хотя бы одна пакетная операция (`_batch_depth > 0`), вызовы `_update_metrics_after_mutation()` пропускаются. Когда глубина возвращается к нулю, выполняется однократный пересчёт метрик.

**Результат:** при вставке/удалении N узлов в пакетном режиме пересчёт метрик выполняется 1 раз вместо N раз, что значительно ускоряет массовые операции.

**Тесты:** 12 новых тестов в `test_pjson_db_batch.cpp` (базовые, вложенные, RAII, смешанные операции, бенчмарк).
...
Все тесты используют Catch2. Текущий набор: 580 тестов, все проходят.
...
| `test_pjson_db_batch.cpp` | Тесты пакетных операций (batch API) |
