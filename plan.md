...
## Фаза 14. Переход на новый менеджер ПАП (`PersistMemoryManager`) ✅ ЗАВЕРШЕНА
...
- [x] Все 717 тестов проекта проходили (до добавления тестов миграции).
...
### Задача 14.9. Утилита миграции старых `.pam` файлов ✅ ВЫПОЛНЕНО

**Цель:** Обеспечить переход для существующих баз данных.

- [x] Создать утилиту `pam_migrate.cpp`:
  ...
  4. Импортировать JSON в новую БД: `node_from_string(json_str, root_id)`.
  5. Сохранить: `pam_pmm_save()`.
- [x] Написать тест миграции: создать БД в старом формате → мигрировать → проверить данные.

**Примечание:** Создан файл `pam_migrate.cpp` — консольная утилита для миграции:
- Загружает старый `.pam` через `pjson_db::open()` (PersistentAddressSpace).
- Экспортирует дерево в JSON через `pjson_db::dump()`.
- Импортирует в новую БД через `pjson_db_pmm::open()` и `node_from_string()`.
- Сохраняет через `pjson_db_pmm::save()`.
- Добавлен как target `pam_migrate` в `CMakeLists.txt`.
- Добавлено 8 тестов в `tests/test_pam_migrate.cpp` (тег `[task14.9][migration]`).
- Тесты покрывают: пустую БД, простые типы, вложенные структуры, `$ref`, `$base64`, Unicode, массовые данные, персистентность.
- Все 725 тестов проходят.

---

### Задача 14.10. Удаление устаревшего кода ✅ ВЫПОЛНЕНО

**Цель:** После успешной миграции всех тестов — удалить старый ПАМ.

- [x] Удалить `pam_core.h` (1500 строк).
- [x] Удалить `persist.h` (374 строки).
- [x] Удалить `pam.h` (88 строк).
- [x] Рефакторинг всех заголовочных файлов: замена `PersistentAddressSpace` API на PMM API.
- [x] Рефакторинг всех тестов: замена `PersistentAddressSpace::Get()` на `pam_pmm_*` функции.
- [x] `pjson_db.h` — тонкая обёртка-алиас на `pjson_db_pmm`.
- [x] `pam_adapter.h` — добавлены `pmm_resolve<T>()` и `pmm_resolve_const<T>()` с поддержкой
  невыровненных (sub-object) смещений через прямую арифметику указателей.
- [x] `pam_migrate.cpp` — обновлён для работы только с PMM API.
- [x] Обновить `CMakeLists.txt` — устаревшие зависимости отсутствовали.
- [x] Финальный прогон всех 713 тестов — все проходят (521 463 assertion).

**Примечания к реализации:**
- `pmm_resolve<T>(off)` использует прямую арифметику `base + off`, а не `offset_to_pptr`,
  чтобы поддерживать невыровненные смещения на подобъекты внутри гранулы (напр. поля структуры).
- `pam_pmm_reset()` теперь корректно устанавливает `initialized = true`.
- Тесты миграции используют `pstringview_manager::reset(); pam_pmm_reset();` вместо
  `pam_pmm_destroy()` для надёжной изоляции между тестами.
...
- [x] PMM подключён как зависимость (single-header или submodule).
- [x] Все персистные типы (`pvector`, `pmap`, `pstring`, `pstringview`) работают через аллокатор PMM.
- [x] `pjson_db::open()` / `save()` используют PMM для управления ПАП.
- [x] Реестр именованных объектов реализован поверх `pmm::pmap`.
- [x] `fptr<T>` = `pptr<T>` (алиас или обёртка).
- [x] Утилита миграции `.pam` → новый формат работает.
- [x] Все 521+ тестов проходят (713 тестов, 521 463 assertion).
- [x] Старый `pam_core.h` / `persist.h` / `pam.h` удалены (Задача 14.10).
...
## Фаза 15. Консолидация двойных реализаций (устранение дублирования _pmm)

**Цель:** После завершения Фазы 14 (полный переход на PMM) в проекте осталось по два файла для каждого основного типа: «оригинальный» `.h` и параллельная `_pmm.h` реализация. Оба варианта уже используют PMM как бэкенд, но через разные API-слои (`pam_pmm_create_array` vs `PamManager::allocate_typed`). Это приводит к 4250+ строк дублированного кода, усложняет поддержку и увеличивает risk рассинхронизации.

Фаза 15 устраняет дублирование, оставляя единственную каноническую реализацию каждого типа.

### Стратегия консолидации

**Принцип:** Файлы `_pmm.h` (`pmem_array_pmm.h`, `pvector_pmm.h`, `pmap_pmm.h`, `pstring_pmm.h`, `pstringview_pmm.h`, `pjson_pool_pmm.h`, `pallocator_pmm.h`) остаются каноническими реализациями. Оригинальные `.h` файлы преобразуются в тонкие обёртки-алиасы, делегирующие в `_pmm.h` файлы.

**Обоснование:**
- `_pmm.h` файлы имеют более лёгкие зависимости (`pam_pmm_config.h`, `pam_adapter.h`) без циклических включений.
- `_pmm.h` файлы используют `PamManager::allocate_typed<T>()` напрямую — без лишней регистрации слотов.
- `_pmm.h` файлы находятся в `namespace pjson` — правильное пространство имён для библиотеки.
- Оригинальные файлы сохраняют имена типов (`pmem_array_hdr`, `pvector<T>`, `pmap<K,V>` и т.д.) для обратной совместимости с `pjson_node.h` и остальной кодовой базой.

### Пары файлов для консолидации

| Оригинал | _pmm дубликат | Слой | Строк (ориг + pmm) |
|----------|---------------|------|---------------------|
| `pmem_array.h` | `pmem_array_pmm.h` | B | 422 + 455 |
| `pvector.h` | `pvector_pmm.h` | B | 205 + 310 |
| `pmap.h` | `pmap_pmm.h` | B | 244 + 478 |
| `pstring.h` | `pstring_pmm.h` | B | 161 + 269 |
| `pstringview.h` | `pstringview_pmm.h` | B | 477 + 274 |
| `pjson_pool.h` | `pjson_pool_pmm.h` | C | 271 + 405 |
| `pallocator.h` | `pallocator_pmm.h` | B | 91 + 188 |

### Порядок консолидации

```
Задача 15.1: pmem_array.h → обёртка над pmem_array_pmm.h (фундамент) ✅
        ↓
  ┌─────┼────────┐
  ↓     ↓        ↓
15.2✅ 15.4✅   15.6✅
(pvector) (pstring) (pallocator)
  ↓     ↓
15.3✅ 15.5✅
(pmap) (pstringview)
  └─────┼────────┘
        ↓
15.7✅: pjson_pool.h → обёртка над pjson_pool_pmm.h
        ↓
15.8✅: Обновить тесты и CI
```

---

### Задача 15.1. Консолидация `pmem_array.h` и `pmem_array_pmm.h` ✅ ВЫПОЛНЕНО

**Цель:** Устранить дублирование кода массивов. Каноническая реализация — `pmem_array_pmm.h`.

**Шаги:**
- [x] Сохранить `pmem_array_pmm.h` как каноническую реализацию (лёгкие зависимости, прямое использование PMM API).
- [x] Преобразовать `pmem_array.h` в тонкую обёртку с `using`-алиасами, делегирующую в `pmem_array_pmm.h`.
- [x] Прогнать все 713 тестов.

**Примечание:** Направление консолидации (оригинал → обёртка, _pmm → каноник) обусловлено
архитектурой зависимостей: `pmem_array.h` включал `fptr_pmm.h` → `pam_pmm.h` → `pmap_pmm.h` →
`pmem_array_pmm.h`, создавая циклическую зависимость при попытке сделать `pmem_array_pmm.h` обёрткой.
`pmem_array_pmm.h` имеет более лёгкую цепочку зависимостей (`pam_pmm_config.h`, `pam_adapter.h`).

**Результат:**
- `pmem_array.h`: 112 строк (обёртка с `using`-алиасами).
- `pmem_array_pmm.h`: 455 строк (каноническая реализация, без изменений).
- Устранено 422 строки дублирования.
- Все 713 тестов проходят.

---

### Задача 15.2. Консолидация `pvector.h` и `pvector_pmm.h` ✅ ВЫПОЛНЕНО

**Цель:** Устранить дублирование кода динамического массива.

- [x] Сохранить `pvector_pmm.h` как каноническую реализацию.
- [x] Преобразовать `pvector.h` в тонкую обёртку с `using`-алиасами.
- [x] Обновить все тесты.
- [x] Прогнать все тесты.

**Примечание:** При консолидации устранены архитектурные различия между реализациями:
- `push_back_direct()` переименован в `push_back()`, двух-аргументный `push_back(val, self_off)` удалён (не использовался).
- Конструктор/деструктор `pvector_pmm` сделаны приватными (как у оригинального `pvector`), доступ через `friend fptr_pmm`.
- Удалён неиспользуемый placeholder `_hdr_off()`.

**Результат:**
- `pvector.h`: 30 строк (обёртка с `using`-алиасом + `#include "fptr_pmm.h"` для обратной совместимости).
- `pvector_pmm.h`: 275 строк (каноническая реализация, упрощён API).
- Устранено ~130 строк дублированного кода.
- Все 713 тестов проходят.

---

### Задача 15.3. Консолидация `pmap.h` и `pmap_pmm.h` ✅ ВЫПОЛНЕНО

**Цель:** Устранить дублирование кода персистной карты.

- [x] Сохранить `pmap_pmm.h` как каноническую реализацию.
- [x] Преобразовать `pmap.h` в тонкую обёртку с `using`-алиасами.
- [x] Обновить тесты.
- [x] Прогнать все тесты.

**Примечание:** При консолидации устранён дублированный код `insert_direct()` (~100 строк):
- Добавлен 2-аргументный `insert(k, v)` в `pmap_pmm`, который вычисляет смещение
  автоматически и делегирует в 3-аргументный `insert(k, v, self_off)`.
- `insert_direct(k, v)` сохранён как псевдоним для обратной совместимости.
- `pjson_node.h` дополнен прямым `#include "pmem_array.h"` (ранее получал транзитивно через `pmap.h`).

**Результат:**
- `pmap.h`: 33 строки (обёртка с `using`-алиасами для `pmap<K,V>` и `pmap_entry<K,V>`).
- `pmap_pmm.h`: 378 строк (каноническая реализация, упрощён API).
- Устранено ~244 строки дублированного кода из `pmap.h`.
- Все 713 тестов проходят.

---

### Задача 15.4. Консолидация `pstring.h` и `pstring_pmm.h` ✅ ВЫПОЛНЕНО

**Цель:** Устранить дублирование кода readwrite-строки.

- [x] Сохранить `pstring_pmm.h` как каноническую реализацию.
- [x] Преобразовать `pstring.h` в тонкую обёртку с `using`-алиасом.
- [x] Обновить тесты (`test_pstring.cpp`: `chars.addr()` → `chars_off`).
- [x] Обновить комментарии в `pjson_node.h` (раскладка `string_val`).
- [x] Прогнать все тесты.

**Примечание:** При консолидации устранены архитектурные различия между реализациями:
- `pstring` использовала `fptr<char> chars` для хранения смещения символьных данных;
  `pstring_pmm` использует `uintptr_t chars_off` напрямую. Раскладки идентичны
  (`fptr<char>` хранит `uintptr_t __addr`), но API доступа к смещению отличается:
  `chars.addr()` → `chars_off`.
- `pstring` имела приватный конструктор/деструктор с `friend fptr_pmm`;
  `pstring_pmm` — тривиально копируемый struct (что корректно для PMM).
- Тесты `test_pstring.cpp` обновлены для использования `chars_off`.

**Результат:**
- `pstring.h`: 29 строк (обёртка с `using`-алиасом + `#include "fptr_pmm.h"` для обратной совместимости).
- `pstring_pmm.h`: 269 строк (каноническая реализация, без изменений).
- Устранено ~130 строк дублированного кода из `pstring.h`.
- Все 713 тестов проходят.

---

### Задача 15.5. Консолидация `pstringview.h` и `pstringview_pmm.h` ✅ ВЫПОЛНЕНО

**Цель:** Устранить дублирование кода readonly-строки и словаря.

- [x] Сохранить `pstringview_pmm.h` как каноническую реализацию.
- [x] Преобразовать `pstringview.h` в тонкую обёртку.
- [x] Обновить тесты.
- [x] Прогнать все тесты.

**Примечание:** Наиболее сложная задача фазы 15. При консолидации:
- `pstringview_pmm.h` расширен полным API: `pstringview_pmm` struct, AVL-дерево обход,
  интернирование, поиск строк, `pstringview_manager` совместимость.
- `pstringview.h` преобразован в обёртку (29 строк): `using pstringview = pjson::pstringview_pmm`.
- Решена проблема персистентности `_root_idx` (корень AVL-дерева): добавлен именованный
  PAM-слот `"pstringview_root"` с ленивым восстановлением через callback-хуки.
- Решена проблема циклической зависимости `pstringview_pmm.h` ↔ `pam_pmm.h`:
  persistence-функции и callback-хуки вынесены в `pam_pmm.h`, связь через function pointers.
- `chars_offset` теперь указывает на `str[]` внутри блока `pmm::pstringview`,
  что обеспечивает корректную резолюцию через `pmm_resolve<char>()`.

**Результат:**
- `pstringview.h`: 29 строк (обёртка с `using`-алиасом).
- `pstringview_pmm.h`: ~370 строк (каноническая реализация с полным API).
- `pam_pmm.h`: +195 строк (persistence-механизм и compatibility API).
- Устранено ~485 строк дублированного кода из `pstringview.h`.
- Все 711 тестов проходят.

---

### Задача 15.6. Консолидация `pallocator.h` и `pallocator_pmm.h` ✅ ВЫПОЛНЕНО

**Цель:** Устранить дублирование STL-аллокатора.

- [x] Сохранить `pallocator_pmm.h` как каноническую реализацию.
- [x] Преобразовать `pallocator.h` в тонкую обёртку.
- [x] Обновить тесты.
- [x] Прогнать все тесты.

**Примечание:** Простейшая задача фазы — `pallocator.h` уже полностью делегировал в
`pallocator_pmm.h`, достаточно было удалить дублированный код и оставить `#include` + `using`.

**Результат:**
- `pallocator.h`: 16 строк (обёртка с `#include` + `using namespace pjson`).
- `pallocator_pmm.h`: без изменений (каноническая реализация).
- Устранено ~94 строки дублированного кода.
- Все 711 тестов проходят.

---

### Задача 15.7. Консолидация `pjson_pool.h` и `pjson_pool_pmm.h` ✅ ВЫПОЛНЕНО

**Цель:** Устранить дублирование кода пула узлов.

- [x] Сохранить `pjson_pool_pmm.h` как каноническую реализацию.
- [x] Преобразовать `pjson_pool.h` в обёртку с member-функциями.
- [x] Обновить тесты.
- [x] Прогнать все тесты.

**Примечание:** `pjson_pool.h` преобразован в struct-обёртку с member-функциями,
делегирующими в `pjson_pool_pmm_*` free-функции. Member-функции (`alloc`, `free`, `get`,
`free_pool`) вычисляют `self_off` через `pam_pmm_ptr_to_offset(this)` и вызывают
каноническую реализацию. Обёртка содержит те же поля, что и `pjson_pool_pmm`,
обеспечивая бинарную совместимость.

**Результат:**
- `pjson_pool.h`: 86 строк (struct-обёртка с member-функциями).
- `pjson_pool_pmm.h`: без изменений (каноническая реализация).
- Устранено ~210 строк дублированного кода.
- Все 711 тестов проходят.

---

### Задача 15.8. Обновить тесты, CI и документацию ✅ ВЫПОЛНЕНО

- [x] Убедиться, что все тесты используют канонические имена типов.
- [x] Обновить `main.cpp` (демонстрация) — без изменений, уже использует `pjson_db_pmm.h`.
- [x] Обновить `plan.md` — критерии приёмки и статус задач.

---

### Критерии приёмки Фазы 15

- [x] Все оригинальные `.h` файлы содержат только `#include` и `using`-алиасы (суммарно < 120 строк каждый).
- [x] `_pmm.h` файлы — канонические реализации, без дублирования логики.
- [x] Нет дублирования логики между оригинальными и _pmm файлами.
- [x] Все 711 тестов проходят.
- [x] CI проходит.
- [x] `plan.md` обновлён.
...
| Риск | Вероятность | Влияние | Смягчение |
|------|-------------|---------|-----------|
| Несовместимость API _pmm и оригинала | Средняя | Среднее | Пошаговая консолидация с тестами на каждом шаге |
| Разная семантика аллокации (fptr vs PamManager) | Низкая | Низкое | Оба уже вызывают PMM; различие только в обёртке |
| Циклические зависимости при объединении | Низкая | Среднее | Сохранение тонких обёрток вместо полного удаления |

---

## Фаза 16. Устранение внутреннего дублирования кода (метапрограммирование)

**Цель:** Устранить повторяющиеся паттерны кода внутри файлов с помощью шаблонных helper-функций
и метапрограммирования (связано с Issue #128). Уменьшить объём кода без изменения API.

**Принципы:**
- Не менять публичный API — все существующие функции сохраняются.
- Внутренняя реализация делегирует в шаблонные helper-функции.
- Код остаётся header-only и C++20.
- Каждая задача — атомарный шаг с прогоном всех тестов.

---

### Задача 16.1. Шаблонный helper для скалярных setter-функций в `pjson_node.h` ✅ ВЫПОЛНЕНО

**Цель:** Устранить дублирование в `node_set_bool`, `node_set_int`, `node_set_uint`, `node_set_real`
и в `node_set_array`, `node_set_object`, `node_set_binary`.

Все эти функции содержат идентичный boilerplate:
```cpp
...
- [x] Добавить шаблонный helper `node_resolve_and_set_tag()` для безопасного разрешения узла и установки тега.
- [x] Добавить шаблонный helper `node_set_container_empty()` для инициализации контейнерных узлов (array/object/binary).
- [x] Делегировать `node_set_bool`, `node_set_int`, `node_set_uint`, `node_set_real` через helper.
- [x] Делегировать `node_set_array`, `node_set_object`, `node_set_binary` через helper.
- [x] Прогнать все тесты.

**Результат:**
- Устранено ~56 строк дублированного boilerplate-кода.
- Все 711 тестов проходят.

---

### Задача 16.2. Извлечение бинарного поиска по объектным ключам ✅ ВЫПОЛНЕНО

**Цель:** Устранить дублирование бинарного поиска в `node_view::at(const char*)`,
`node_object_insert()` и `pjson_db_pmm::_object_erase()`.

- [x] Добавить helper-функцию `node_object_find_key()` — бинарный поиск по ключу в массиве `object_entry`.
- [x] Добавить общие функторы `object_entry_key_of` / `object_entry_less` (вместо локальных ObjKeyOf/ObjLess).
- [x] Делегировать все три реализации через этот helper.
- [x] Делегировать PMM-версии tree traversal (`pjson_pmm_count_nodes_in_subtree`, `pjson_pmm_search_node_strings_in_subtree`) в оригиналы из `pjson_db_helpers.h`.
- [x] Прогнать все тесты.

**Результат:**
- Устранено ~44 строки дублированного бинарного поиска (3 реализации → 1 helper).
- Устранено ~100 строк дублированного tree traversal (PMM-версии делегируют в оригиналы).
- Устранено дублирование функторов ObjKeyOf/ObjLess (2 копии → 1 пара на уровне файла).
- Все 586 тестов проходят.

---

### Задача 16.3. Шаблонный helper для безопасного разрешения узла с проверкой тега ✅ ВЫПОЛНЕНО

**Цель:** Устранить повторяющийся паттерн guard-and-check в `pjson_db_pmm.h`:
```cpp
node* n = pmm_resolve<node>( id );
if ( n == nullptr ) return false;
if ( n->tag != expected_tag ) return false;
```

- [x] Добавить `node_resolve_checked()` — helper для разрешения с проверкой тега.
- [x] Делегировать повторяющиеся паттерны через helper (`_object_erase`, `_array_erase_at`, `_object_insert_direct`, `_array_set_direct`).
- [x] Прогнать все тесты.

**Результат:**
- Устранено ~12 строк дублированного guard-паттерна (4 метода → единый helper).
- Все 586 тестов проходят.

---

### Задача 16.4. Реестр метрик вместо цепочки strcmp ✅ ВЫПОЛНЕНО

**Цель:** Устранить повторяющиеся `if (strcmp(key, "metric_name") == 0)` в `pjson_db_pmm.h`.

- [x] Создать массив-реестр метрик: пара (имя, функция-геттер для чтения значения).
- [x] Заменить цепочку strcmp на цикл по реестру.
- [x] Добавить `subtree_counts` с ленивым вычислением для группы subtree-метрик.
- [x] Прогнать все тесты.

**Результат:**
- Заменена цепочка из 14+ strcmp-вызовов на декларативный реестр из 14 записей.
- Добавление новых метрик — одна строка в реестре вместо if-else блока.
- Все 586 тестов проходят.

---

### Задача 16.5. Обновить тесты, CI и документацию ✅ ВЫПОЛНЕНО

- [x] Убедиться, что все тесты проходят.
- [x] Обновить `readme.md` (добавить Фазу 16 в таблицу характеристик).
- [x] Обновить `plan.md` — критерии приёмки и статус задач.

---

### Критерии приёмки Фазы 16 ✅

- [x] Все шаблонные helpers — `inline` в header-файлах (header-only).
- [x] Публичный API не изменён — все существующие функции сохранены.
- [x] Нет дублирования boilerplate-кода в `node_set_*` функциях.
- [x] Бинарный поиск по объектным ключам — единственная реализация.
- [x] Все 586 тестов проходят.
- [x] CI проходит.
- [x] `plan.md` обновлён.
...
| Риск | Вероятность | Влияние | Смягчение |
|------|-------------|---------|-----------|
| Изменение семантики при рефакторинге | Низкая | Среднее | Пошаговый рефакторинг с прогоном тестов |
| Проблемы с инлайнингом шаблонов | Низкая | Низкое | Все helpers — inline, аналогично существующему коду |
| Неожиданное влияние на производительность | Низкая | Низкое | Перфоманс-тесты (Фаза 9) покажут отклонения |
