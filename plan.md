...
(pmap) (pstringview)
  └─────┼────────┘
        ↓
Задача 15.7: pjson_pool.h → обёртка над pjson_pool_pmm.h
        ↓
Задача 15.8: Обновить тесты и CI
```
...
### Задача 15.5. Консолидация `pstringview.h` и `pstringview_pmm.h`

**Цель:** Устранить дублирование кода readonly-строки и словаря.

- [ ] Сохранить `pstringview_pmm.h` как каноническую реализацию.
- [ ] Преобразовать `pstringview.h` в тонкую обёртку.
- [ ] Обновить тесты.
- [ ] Прогнать все тесты.

---

### Задача 15.6. Консолидация `pallocator.h` и `pallocator_pmm.h`

**Цель:** Устранить дублирование STL-аллокатора.

- [ ] Сохранить `pallocator_pmm.h` как каноническую реализацию.
- [ ] Преобразовать `pallocator.h` в тонкую обёртку.
- [ ] Обновить тесты.
- [ ] Прогнать все тесты.

---

### Задача 15.7. Консолидация `pjson_pool.h` и `pjson_pool_pmm.h`

**Цель:** Устранить дублирование кода пула узлов.

- [ ] Сохранить `pjson_pool_pmm.h` как каноническую реализацию.
- [ ] Преобразовать `pjson_pool.h` в тонкую обёртку.
- [ ] Обновить `pjson_db_pmm.h` для использования единого пула.
- [ ] Обновить тесты.
- [ ] Прогнать все тесты.

---

### Задача 15.8. Обновить тесты, CI и документацию

- [ ] Убедиться, что все тесты используют канонические имена типов.
- [ ] Обновить `main.cpp` (демонстрация).
- [ ] Обновить `readme.md` — раздел архитектуры.
- [ ] Обновить `plan.md` — критерии приёмки.

---

### Критерии приёмки Фазы 15

- [ ] Все оригинальные `.h` файлы содержат только `#include` и `using`-алиасы (суммарно < 120 строк каждый).
- [ ] `_pmm.h` файлы — канонические реализации, без дублирования логики.
- [ ] Нет дублирования логики между оригинальными и _pmm файлами.
- [ ] Все 713+ тестов проходят.
- [ ] CI проходит.
- [ ] `readme.md` обновлён.
