...
- Итого: 482 теста проходят (475 предыдущих + 7 новых).
...
- Старые файлы (`pjson_serializer.h`, `pjson_interning.h`, `pjson_node_pool.h`) сохранены,
  т.к. используются существующими тестами (`test_pjson.cpp`, `test_pjson_bench.cpp`,
  `test_pjson_serial.cpp`, `test_pjson_opt.cpp`, `test_pjson_large.cpp`).
- `pjson.h` сохранён как legacyAPI для совместимости с указанными тестами.
- Новый код (`pjson_db.h`) не зависит от устаревших файлов.
- Полное удаление — задача будущей фазы после миграции тестов.
