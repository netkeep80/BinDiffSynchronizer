...
|------|-----------|--------|
| M9  | `slot_descriptor` определён и задокументирован | ⬜ Ожидает |
| M10 | `PersistentAddressSpace` реализован: Init, Create, Resolve, Find, Save | ⬜ Ожидает |
| M11 | `fptr<T>` рефакторинг: `sizeof(fptr<T>)==sizeof(void*)`, удалена файловая логика | ⬜ Ожидает |
| M12 | `persist<T>` рефакторинг: `sizeof(persist<T>)==sizeof(T)`, удалена файловая логика | ⬜ Ожидает |
| M13 | Тонкий адаптер `AddressManager<T>` над ПАП | ⬜ Ожидает |
| M14 | Все тесты Phase 1 проходят с новым ПАП | ⬜ Ожидает |
| M15 | `test_pam.cpp` — новые тесты для ПАП проходят | ⬜ Ожидает |
| M16 | Все комментарии переведены на русский язык | ⬜ Ожидает |
| M17 | Обновлены `readme.md` и `DEVELOPMENT_PLAN.md` | ⬜ Ожидает |
...
1. **Размер образа ПАП**: Каков максимальный размер образа (в мегабайтах)? Влияет на стратегию выделения (`fread`/`mmap`).
2. **Конкурентный доступ**: Требуется ли поддержка многопоточного доступа к ПАП в Phase 2, или это Phase 3?
3. **Шифрование и архивация**: Требование упоминает возможность хранения в зашифрованном архиве — это Phase 2 или Phase 3?
4. **Фрагментация памяти**: Нужен ли дефрагментатор для области данных ПАП, или достаточно простого bump-allocator на Phase 2?
5. **Обратная совместимость файлов Phase 1**: Нужна ли миграция существующих `.extend`-файлов Phase 1 в новый формат `.pap`?
